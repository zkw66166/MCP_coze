...
class ChatWidget(QWidget):
    ...
    def append_assistant_content(self, content: str, is_final=False):
        ...
        if is_final:
            ...
            if self.current_answer_text:
                ...
                self.chat_display.append(f'<div class="assistant-answer">{html_content}</div>')
                self.current_answer_text = ""
        else:
            # 实时更新：累积纯文本，直接显示纯文本（不做Markdown转换）
            self.current_answer_text += content
            cursor = self.chat_display.textCursor()
            cursor.movePosition(QTextCursor.MoveOperation.End)
            self.chat_display.setTextCursor(cursor)
            # 直接插入纯文本，不做Markdown转换，避免频繁换行
    ...
    def append_system_message(self, message: str, is_error=False):
        ...
        html = f"""
            <div style="color: {color}; padding: 10px; background: #f9fafb; border-radius: 5px; margin: 10px 0; border-left: 4px solid {color};">
