...
请直接返回企业名称或"无法识别"。"""
    ...
    def execute_query(self, company_id: int, time_range: Dict, metrics: List[str], 
        ...
        """
        # === 复杂条件检测 ===
        # 如果问题包含字段过滤条件(如"卖方名称为..."),应使用Text-to-SQL
        ...
        if question:
            import re as cond_re
            for pattern in complex_condition_patterns:
                if cond_re.search(pattern, question):
                    has_complex_condition = True
                    break
        
        # 如果有复杂条件,直接使用Text-to-SQL
        if has_complex_condition and question:
            print(f"🔍 检测到复杂过滤条件,使用Text-to-SQL")
            try:
